...
class Settings(BaseSettings):
    ...
    SSL_MODE: Literal["disable", "allow", "prefer", "require", "verify-ca", "verify-full"] = Field(
        "prefer", validation_alias="SSL_MODE"
    )
    ...
    BACKEND_CORS_ORIGINS: list[str] | str = Field(["*"], validation_alias="BACKEND_CORS_ORIGINS")

    @field_validator("SSL_MODE", "DATABASE_URL", "SECRET_KEY", "ALGORITHM", "ENV", mode="before")
    @classmethod
